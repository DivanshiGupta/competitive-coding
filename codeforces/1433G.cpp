...
int32_t main()
{
    ...
    for(int i=0;i<n;i++)
    {
		...
		while(!que.empty())
		{
			...
			for(auto it:gr[u])
			{
				if(dist[i][it.first] > (dist[i][u]+it.second))
				{
					dist[i][it.first]=dist[i][u]+it.second;
					que.push({dist[i][it.first],it.first});
				}
			}
		}
    }
    ...
    for(int i=0;i<k;i++)
    {
    	ans+=dist[vec[i].first][vec[i].second];
    }
    int fans=ans;
    for(int i=0;i<m;i++)
    {
    	...
    	for(int j=0;j<k;j++)
    	{
    		int d=min(min(dist[vec[j].first][a]+dist[b][vec[j].second],
    			dist[vec[j].first][b]+dist[a][vec[j].second]),dist[vec[j].first][vec[j].second]);
    		ans1+=d-dist[vec[j].first][vec[j].second];
    	}
    	fans=min(ans1,fans);
    }
    cout<<fans<<endl;
